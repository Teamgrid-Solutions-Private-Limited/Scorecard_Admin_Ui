...
import { FIELD_LABELS } from "./constants/senatorConstants";
import {
  validateVoteInTermRange as validateVoteInTermRangeUtil,
  validateActivityInTermRange as validateActivityInTermRangeUtil,
  doesVoteBelongToTerm,
  doesActivityBelongToTerm,
  compareValues,
  formatRelativeTime,
  sanitizeKey,
} from "./utils/validationUtils";
import {
  getAvailableTerms as getAvailableTermsUtil,
  getValidTermId,
  getFieldDisplayName as getFieldDisplayNameUtil,
...
export default function AddSenator(props) {
  ...
  // Validation functions now imported from utils
  const validateVoteInTermRange = (voteId, termId) => {
    return validateVoteInTermRangeUtil(voteId, termId, allVotes, terms);
  };

  const validateActivityInTermRange = (activityId, termId) => {
    return validateActivityInTermRangeUtil(activityId, termId, allActivities, terms);
  };
  ...
  // Wrapper functions that use imported utilities with component state
  const getAvailableTerms = (currentTermIndex) => {
    return getAvailableTermsUtil(currentTermIndex, senatorTermData, terms);
  };

  const getFieldDisplayName = (field) => {
    return getFieldDisplayNameUtil(field, fieldLabels);
  };
  ...
  // compareValues now imported from utils
  const isDataReady = () => {
    return (
      !isInitialLoad &&
      dataLoaded &&
        ...
        if (orphanVotes.length > 0) {
          ...
          orphanVotes.forEach((orphanVote) => {
            if (orphanVote.voteId && orphanVote.voteId !== "") {
              ...
              if (voteData) {
                // sanitizeKey now imported from utils
  ...
  // getValidTermId now imported from utils - wrapper to pass terms
  const getValidTermIdWrapper = (termId) => getValidTermId(termId, terms);
            ...
            {senatorTermData.map((term, termIndex) => (
              ...
              >
                <SenatorTermSection
                  ...
                  getValidTermId={getValidTermIdWrapper}
