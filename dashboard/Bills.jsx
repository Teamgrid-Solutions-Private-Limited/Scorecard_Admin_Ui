...
export default function Bills(props) {
    ...
    const { votes, loading } = useSelector((state) => state.vote); // Corrected from state.votes to state.vote
    ...
    const billsData = votes.map((vote, index) => ({
        _id: vote._id || index, // Ensure this is unique
        date: vote.date,
        bill: vote.billName || vote.title, // Use the correct field for the bill name
    }));

    console.log("Mapped bills data:", billsData); // Debug the mapped data
            ...
            <Box sx={{ display: "flex" }}>
                ...
                <Box sx={{ flexGrow: 1, overflow: "auto" }}>
                    <Stack spacing={2} sx={{ alignItems: "center", mx: 3, pb: 5, mt: { xs: 8, md: 0 } }}>
                        ...
                        <MainGrid type="bills" data={billsData} loading={loading} />
                    </Stack>
                </Box>
            </Box>
        </AppTheme>
